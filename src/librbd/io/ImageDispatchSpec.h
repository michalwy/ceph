...
public:
  ...
  typedef boost::variant<Read, Discard, Write, WriteSame, CompareAndWrite,
                         Flush, ListSnaps>
      Request;
  ...
  static ImageDispatchSpec *
  create_read(ImageCtxT &image_ctx, ImageDispatchLayer image_dispatch_layer,
              AioCompletion *aio_comp, Extents &&image_extents,
              ReadResult &&read_result, IOContext io_context, int op_flags,
              int read_flags, const ZTracer::Trace &parent_trace) {
    LT_CP();
    return new ImageDispatchSpec(
        image_ctx.io_image_dispatcher, image_dispatch_layer, aio_comp,
        std::move(image_extents), Read{std::move(read_result), read_flags},
        io_context, op_flags, parent_trace);
  }

  template <typename ImageCtxT = ImageCtx>
  static ImageDispatchSpec *
  create_discard(ImageCtxT &image_ctx, ImageDispatchLayer image_dispatch_layer,
    ...
    return new ImageDispatchSpec(image_ctx.io_image_dispatcher,
                                 ...
                                 Discard{discard_granularity_bytes}, io_context,
                                 0, parent_trace);
  }

  template <typename ImageCtxT = ImageCtx>
  static ImageDispatchSpec *
  create_write(ImageCtxT &image_ctx, ImageDispatchLayer image_dispatch_layer,
               AioCompletion *aio_comp, Extents &&image_extents,
  ...
  static ImageDispatchSpec *create_write_same(
      ...
      AioCompletion *aio_comp, uint64_t off, uint64_t len, bufferlist &&bl,
      IOContext io_context, int op_flags, const ZTracer::Trace &parent_trace) {
    return new ImageDispatchSpec(image_ctx.io_image_dispatcher,
                                 image_dispatch_layer, aio_comp, {{off, len}},
                                 WriteSame{std::move(bl)}, io_context, op_flags,
                                 parent_trace);
  }
  ...
  static ImageDispatchSpec *create_compare_and_write(
    ...
    return new ImageDispatchSpec(
        ...
        CompareAndWrite{std::move(cmp_bl), std::move(bl), mismatch_offset},
        io_context, op_flags, parent_trace);
  }

  template <typename ImageCtxT = ImageCtx>
  static ImageDispatchSpec *
  create_flush(ImageCtxT &image_ctx, ImageDispatchLayer image_dispatch_layer,
  ...
  static ImageDispatchSpec *create_list_snaps(
    ...
    return new ImageDispatchSpec(
        image_ctx.io_image_dispatcher, image_dispatch_layer, aio_comp,
        std::move(image_extents),
        ListSnaps{std::move(snap_ids), list_snaps_flags, snapshot_delta}, {}, 0,
        parent_trace);
  }

  ~ImageDispatchSpec() { aio_comp->put(); }
