...
#undef dout_prefix
#define dout_prefix                                                            \
  *_dout << "librbd::io::ObjectDispatch: " << this << " " << __func__ << ": "
...
template <typename I>
ObjectDispatch<I>::ObjectDispatch(I *image_ctx) : m_image_ctx(image_ctx) {}

template <typename I> void ObjectDispatch<I>::shut_down(Context *on_finish) {
...
template <typename I>
bool ObjectDispatch<I>::read(uint64_t object_no, ReadExtents *extents,
...
template <typename I>
bool ObjectDispatch<I>::discard(uint64_t object_no, uint64_t object_off,
                                uint64_t object_len, IOContext io_context,
                                int discard_flags,
                                const ZTracer::Trace &parent_trace,
...
template <typename I>
bool ObjectDispatch<I>::write(uint64_t object_no, uint64_t object_off,
                              ceph::bufferlist &&data, IOContext io_context,
                              int op_flags, int write_flags,
  ...
  *dispatch_result = DISPATCH_RESULT_COMPLETE;
  auto req = new ObjectWriteRequest<I>(
      m_image_ctx, object_no, object_off, std::move(data), io_context, op_flags,
      write_flags, assert_version, parent_trace, on_dispatched);
  req->send();
  return true;
}
...
bool ObjectDispatch<I>::write_same(
  ...
  *dispatch_result = DISPATCH_RESULT_COMPLETE;
  auto req = new ObjectWriteSameRequest<I>(
      m_image_ctx, object_no, object_off, object_len, std::move(data),
      io_context, op_flags, parent_trace, on_dispatched);
  req->send();
  return true;
}
...
bool ObjectDispatch<I>::compare_and_write(
  ...
  *dispatch_result = DISPATCH_RESULT_COMPLETE;
  auto req = new ObjectCompareAndWriteRequest<I>(
      m_image_ctx, object_no, object_off, std::move(cmp_data),
      std::move(write_data), io_context, mismatch_offset, op_flags,
      parent_trace, on_dispatched);
  req->send();
  return true;
}
...
template <typename I>
bool ObjectDispatch<I>::list_snaps(uint64_t object_no, io::Extents &&extents,
                                   SnapIds &&snap_ids, int list_snap_flags,
                                   const ZTracer::Trace &parent_trace,
