...
struct UserBufferDeleter : public deleter::impl {
  ...
  ~UserBufferDeleter() override { aio_completion->unblock(cct); }
};
...
static auto create_write_raw(librbd::ImageCtx *ictx, const char *buf,
  ...
  return ceph::unique_leakable_ptr<ceph::buffer::raw>(buffer::claim_buffer(
...
get_aio_completion(librbd::RBD::AioCompletion *comp) {
...
struct C_AioCompletion : public Context {
  ...
  virtual ~C_AioCompletion() { aio_comp->put(); }
...
struct C_OpenComplete : public C_AioCompletion {
  ...
  C_OpenComplete(librbd::ImageCtx *ictx, librbd::io::AioCompletion *comp,
                 void **ictxp)
      : C_AioCompletion(ictx, librbd::io::AIO_TYPE_OPEN, comp), ictx(ictx),
        ictxp(ictxp) {}
  void finish(int r) override {
...
struct C_OpenAfterCloseComplete : public Context {
  ...
  C_OpenAfterCloseComplete(librbd::ImageCtx *ictx,
                           librbd::io::AioCompletion *comp, void **ictxp)
...
struct C_UpdateWatchCB : public librbd::UpdateWatchCtx {
  ...
  void handle_notify() override { watch_cb(arg); }
};
...
struct C_QuiesceWatchCB : public librbd::QuiesceWatchCtx {
  ...
  void handle_quiesce() override { quiesce_cb(arg); }
  void handle_unquiesce() override { unquiesce_cb(arg); }
};
...
int get_local_mirror_image_site_status(
  ...
  auto it = std::find_if(
      status.site_statuses.begin(), status.site_statuses.end(),
  ...
  void finish(int r) override {
    ...
    if (mirror_image_status != nullptr) {
      ...
      r = get_local_mirror_image_site_status(cpp_mirror_image_global_status,
                                             &local_status);
      if (r >= 0) {
        *mirror_image_status_cpp = {cpp_mirror_image_global_status.name,
                                    cpp_mirror_image_global_status.info,
                                    local_status.state,
                                    local_status.description,
 ...
 * Pool stats
 */
PoolStats::PoolStats() { rbd_pool_stats_create(&pool_stats); }
...
RBD::~RBD() {
  LT_CP();
  lat_tracker.stop("librbd_lt.txt");
}
...
int RBD::aio_open(IoCtx &io_ctx, Image &image, const char *name,
  ...
  tracepoint(librbd, aio_open_image_enter, ictx, ictx->name.c_str(),
             ictx->id.c_str(), ictx->snap_name.c_str(), ictx->read_only, c->pc);
  ...
  if (image.ctx != NULL) {
    ...
    ictx->state->open(
        0, new C_OpenComplete(ictx, get_aio_completion(c), &image.ctx));
  }
...
int RBD::features_from_string(const std::string str_features,
                              uint64_t *features) {
  std::stringstream err;
...
int RBD::create(IoCtx &io_ctx, const char *name, uint64_t size, int *order) {
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
  tracepoint(librbd, create_enter, io_ctx.get_pool_name().c_str(),
             io_ctx.get_id(), name, size, *order);
...
int RBD::create2(IoCtx &io_ctx, const char *name, uint64_t size,
  ...
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
  tracepoint(librbd, create2_enter, io_ctx.get_pool_name().c_str(),
             io_ctx.get_id(), name, size, features, *order);
...
int RBD::create3(IoCtx &io_ctx, const char *name, uint64_t size,
  ...
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
  tracepoint(librbd, create3_enter, io_ctx.get_pool_name().c_str(),
             io_ctx.get_id(), name, size, features, *order, stripe_unit,
             stripe_count);
...
int RBD::create4(IoCtx &io_ctx, const char *name, uint64_t size,
                 ImageOptions &opts) {
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
  tracepoint(librbd, create4_enter, io_ctx.get_pool_name().c_str(),
             io_ctx.get_id(), name, size, opts.opts);
...
int RBD::clone(IoCtx &p_ioctx, const char *p_name, const char *p_snap_name,
  ...
  tracepoint(librbd, clone_enter, p_ioctx.get_pool_name().c_str(),
             p_ioctx.get_id(), p_name, p_snap_name,
             c_ioctx.get_pool_name().c_str(), c_ioctx.get_id(), c_name,
             features);
  int r = librbd::clone(p_ioctx, p_name, p_snap_name, c_ioctx, c_name, features,
                        c_order, 0, 0);
...
int RBD::clone2(IoCtx &p_ioctx, const char *p_name, const char *p_snap_name,
  ...
  TracepointProvider::initialize<tracepoint_traits>(get_cct(p_ioctx));
  tracepoint(librbd, clone2_enter, p_ioctx.get_pool_name().c_str(),
             p_ioctx.get_id(), p_name, p_snap_name,
             c_ioctx.get_pool_name().c_str(), c_ioctx.get_id(), c_name,
             features, stripe_unit, stripe_count);
  int r = librbd::clone(p_ioctx, p_name, p_snap_name, c_ioctx, c_name, features,
...
int RBD::clone3(IoCtx &p_ioctx, const char *p_name, const char *p_snap_name,
  ...
  TracepointProvider::initialize<tracepoint_traits>(get_cct(p_ioctx));
  tracepoint(librbd, clone3_enter, p_ioctx.get_pool_name().c_str(),
             p_ioctx.get_id(), p_name, p_snap_name,
             c_ioctx.get_pool_name().c_str(), c_ioctx.get_id(), c_name,
             c_opts.opts);
  int r = librbd::clone(p_ioctx, nullptr, p_name, p_snap_name, c_ioctx, nullptr,
                        c_name, c_opts, "", "");
...
int RBD::remove(IoCtx &io_ctx, const char *name) {
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
  tracepoint(librbd, remove_enter, io_ctx.get_pool_name().c_str(),
             io_ctx.get_id(), name);
...
int RBD::trash_list(IoCtx &io_ctx, vector<trash_image_info_t> &entries) {
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
  tracepoint(librbd, trash_list_enter, io_ctx.get_pool_name().c_str(),
             io_ctx.get_id());
...
int RBD::rename(IoCtx &src_io_ctx, const char *srcname, const char *destname) {
  ...
  tracepoint(librbd, rename_enter, src_io_ctx.get_pool_name().c_str(),
             src_io_ctx.get_id(), srcname, destname);
...
int RBD::migration_execute_with_progress(IoCtx &io_ctx, const char *image_name,
...
int RBD::mirror_site_name_get(librados::Rados &rados, std::string *site_name) {
...
int RBD::mirror_peer_site_add(IoCtx &io_ctx, std::string *uuid,
  ...
  return librbd::api::Mirror<>::peer_site_add(io_ctx, uuid, direction,
...
int RBD::mirror_peer_site_set_direction(IoCtx &io_ctx, const std::string &uuid,
                                        mirror_peer_direction_t direction) {
...
int RBD::mirror_image_global_status_list(
  ...
  return librbd::api::Mirror<>::image_global_status_list(io_ctx, start_id, max,
                                                         global_statuses);
}
...
int RBD::mirror_peer_list(IoCtx &io_ctx, std::vector<mirror_peer_t> *peers) {
  ...
  for (auto &peer_site : peer_sites) {
    peers->push_back(
        {peer_site.uuid, peer_site.site_name, peer_site.client_name});
  }
  return 0;
}
...
int RBD::mirror_peer_set_cluster(IoCtx &io_ctx, const std::string &uuid,
                                 const std::string &cluster_name) {
  return librbd::api::Mirror<>::peer_site_set_name(io_ctx, uuid, cluster_name);
}
...
int RBD::mirror_image_status_list(
  ...
  int r = librbd::api::Mirror<>::image_global_status_list(io_ctx, start_id, max,
                                                          &global_statuses);
  if (r < 0) {
    return r;
  }
...
int RBD::mirror_image_status_summary(
    IoCtx &io_ctx, std::map<mirror_image_status_state_t, int> *states) {
  return librbd::api::Mirror<>::image_status_summary(io_ctx, states);
}

int RBD::mirror_image_instance_id_list(
    IoCtx &io_ctx, const std::string &start_id, size_t max,
...
int RBD::mirror_image_info_list(
    ...
    std::map<std::string, std::pair<mirror_image_mode_t, mirror_image_info_t>>
        *entries) {
...
int RBD::group_image_add(IoCtx &group_ioctx, const char *group_name,
  ...
  tracepoint(librbd, group_image_add_enter, group_ioctx.get_pool_name().c_str(),
             ...
             image_ioctx.get_pool_name().c_str(), image_ioctx.get_id(),
             image_name);
...
int RBD::group_image_remove(IoCtx &group_ioctx, const char *group_name,
  ...
  tracepoint(librbd, group_image_remove_enter,
             ...
             group_name, image_ioctx.get_pool_name().c_str(),
...
RBD::AioCompletion::AioCompletion(void *cb_arg, callback_t complete_cb) {
  auto aio_comp = librbd::io::AioCompletion::create(cb_arg, complete_cb, this);
...
ImageOptions::ImageOptions() { librbd::image_options_create(&opts); }
...
ImageOptions::~ImageOptions() { librbd::image_options_destroy(opts); }
...
void ImageOptions::clear() { librbd::image_options_clear(opts); }
...
Image::Image() : ctx(NULL) { std::cout << __func__ << std::endl; }
...
int Image::close() {
  ...
  if (ctx) {
    ...
    tracepoint(librbd, close_image_enter, ictx, ictx->name.c_str(),
               ictx->id.c_str());
...
int Image::aio_close(RBD::AioCompletion *c) {
  ...
  ImageCtx *ictx = (ImageCtx *)ctx;
  tracepoint(librbd, aio_close_image_enter, ictx, ictx->name.c_str(),
             ictx->id.c_str(), c->pc);
...
int Image::resize(uint64_t size) {
  ImageCtx *ictx = (ImageCtx *)ctx;
  tracepoint(librbd, resize_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only, size);
  librbd::NoOpProgressContext prog_ctx;
...
int Image::resize2(uint64_t size, bool allow_shrink,
...
int Image::stat(image_info_t &info, size_t infosize) {
  ImageCtx *ictx = (ImageCtx *)ctx;
  tracepoint(librbd, stat_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only);
...
int Image::old_format(uint8_t *old) {
  ImageCtx *ictx = (ImageCtx *)ctx;
  tracepoint(librbd, get_old_format_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only);
...
int Image::size(uint64_t *size) {
  ImageCtx *ictx = (ImageCtx *)ctx;
  tracepoint(librbd, get_size_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only);
...
int Image::features(uint64_t *features) {
  ImageCtx *ictx = (ImageCtx *)ctx;
  tracepoint(librbd, get_features_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only);
...
uint64_t Image::get_stripe_unit() const {
  ImageCtx *ictx = (ImageCtx *)ctx;
  tracepoint(librbd, get_stripe_unit_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only);
...
uint64_t Image::get_stripe_count() const {
  ImageCtx *ictx = (ImageCtx *)ctx;
  tracepoint(librbd, get_stripe_count_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only);
...
int Image::overlap(uint64_t *overlap) {
  ImageCtx *ictx = (ImageCtx *)ctx;
  tracepoint(librbd, get_overlap_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only);
...
int Image::get_parent(linked_image_spec_t *parent_image,
  ...
  tracepoint(librbd, get_parent_info_exit, r, parent_image->pool_name.c_str(),
             parent_image->image_name.c_str(), parent_image->image_id.c_str(),
...
int Image::copy(IoCtx &dest_io_ctx, const char *destname) {
  ImageCtx *ictx = (ImageCtx *)ctx;
  tracepoint(librbd, copy_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only,
...
int Image::copy2(Image &dest) {
  ...
  tracepoint(librbd, copy2_enter, srcctx, srcctx->name.c_str(),
             srcctx->snap_name.c_str(), srcctx->read_only, destctx,
             destctx->name.c_str(), destctx->snap_name.c_str(),
             destctx->read_only);
  librbd::NoOpProgressContext prog_ctx;
...
int Image::copy3(IoCtx &dest_io_ctx, const char *destname, ImageOptions &opts) {
  ImageCtx *ictx = (ImageCtx *)ctx;
  tracepoint(librbd, copy3_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only,
             dest_io_ctx.get_pool_name().c_str(), dest_io_ctx.get_id(),
             destname, opts.opts);
  librbd::NoOpProgressContext prog_ctx;
...
int Image::copy4(IoCtx &dest_io_ctx, const char *destname, ImageOptions &opts,
                 size_t sparse_size) {
  ImageCtx *ictx = (ImageCtx *)ctx;
  tracepoint(librbd, copy4_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only,
             dest_io_ctx.get_pool_name().c_str(), dest_io_ctx.get_id(),
             destname, opts.opts, sparse_size);
  librbd::NoOpProgressContext prog_ctx;
  int r =
      librbd::copy(ictx, dest_io_ctx, destname, opts, prog_ctx, sparse_size);
...
int Image::list_children2(vector<librbd::child_info_t> *children) {
  ...
  for (auto &image : images) {
    children->push_back({.pool_name = image.pool_name,
...
int Image::list_lockers(std::list<librbd::locker_t> *lockers, bool *exclusive,
                        string *tag) {
  ImageCtx *ictx = (ImageCtx *)ctx;
  tracepoint(librbd, list_lockers_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only);
  int r = librbd::list_lockers(ictx, lockers, exclusive, tag);
  if (r >= 0) {
    for (std::list<librbd::locker_t>::const_iterator it = lockers->begin();
         it != lockers->end(); ++it) {
      tracepoint(librbd, list_lockers_entry, it->client.c_str(),
                 it->cookie.c_str(), it->address.c_str());
    }
  }
...
int Image::lock_exclusive(const string &cookie) {
  ImageCtx *ictx = (ImageCtx *)ctx;
  tracepoint(librbd, lock_exclusive_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only, cookie.c_str());
...
int Image::lock_shared(const string &cookie, const std::string &tag) {
  ImageCtx *ictx = (ImageCtx *)ctx;
  tracepoint(librbd, lock_shared_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only, cookie.c_str(),
             tag.c_str());
...
int Image::unlock(const string &cookie) {
  ImageCtx *ictx = (ImageCtx *)ctx;
  tracepoint(librbd, unlock_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only, cookie.c_str());
...
int Image::break_lock(const string &client, const string &cookie) {
  ImageCtx *ictx = (ImageCtx *)ctx;
  tracepoint(librbd, break_lock_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only, client.c_str(),
             cookie.c_str());
...
int Image::snap_create(const char *snap_name) {
  ImageCtx *ictx = (ImageCtx *)ctx;
  tracepoint(librbd, snap_create_enter, ictx, ictx->name.c_str(),
...
int Image::snap_remove(const char *snap_name) {
  ImageCtx *ictx = (ImageCtx *)ctx;
  tracepoint(librbd, snap_remove_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only, snap_name);
  librbd::NoOpProgressContext prog_ctx;
...
int Image::snap_remove2(const char *snap_name, uint32_t flags,
                        ProgressContext &pctx) {
  ImageCtx *ictx = (ImageCtx *)ctx;
  tracepoint(librbd, snap_remove2_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only, snap_name, flags);
...
int Image::snap_rollback(const char *snap_name) {
  ImageCtx *ictx = (ImageCtx *)ctx;
  tracepoint(librbd, snap_rollback_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only, snap_name);
  librbd::NoOpProgressContext prog_ctx;
  int r = ictx->operations->snap_rollback(cls::rbd::UserSnapshotNamespace(),
                                          snap_name, prog_ctx);
...
int Image::snap_rename(const char *srcname, const char *dstname) {
  ImageCtx *ictx = (ImageCtx *)ctx;
  tracepoint(librbd, snap_rename_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only, srcname, dstname);
...
int Image::snap_protect(const char *snap_name) {
  ImageCtx *ictx = (ImageCtx *)ctx;
  tracepoint(librbd, snap_protect_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only, snap_name);
  int r = ictx->operations->snap_protect(cls::rbd::UserSnapshotNamespace(),
                                         snap_name);
...
int Image::snap_unprotect(const char *snap_name) {
  ImageCtx *ictx = (ImageCtx *)ctx;
  tracepoint(librbd, snap_unprotect_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only, snap_name);
  int r = ictx->operations->snap_unprotect(cls::rbd::UserSnapshotNamespace(),
                                           snap_name);
...
int Image::snap_is_protected(const char *snap_name, bool *is_protected) {
  ImageCtx *ictx = (ImageCtx *)ctx;
  tracepoint(librbd, snap_is_protected_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only, snap_name);
...
int Image::snap_list(vector<librbd::snap_info_t> &snaps) {
  ImageCtx *ictx = (ImageCtx *)ctx;
  tracepoint(librbd, snap_list_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only, &snaps);
  int r = librbd::api::Snapshot<>::list(ictx, snaps);
  if (r >= 0) {
    for (int i = 0, n = snaps.size(); i < n; i++) {
      tracepoint(librbd, snap_list_entry, snaps[i].id, snaps[i].size,
                 snaps[i].name.c_str());
    }
  }
...
bool Image::snap_exists(const char *snap_name) {
  ...
  tracepoint(librbd, snap_exists_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only, snap_name);
  bool exists;
  int r = librbd::api::Snapshot<>::exists(
      ictx, cls::rbd::UserSnapshotNamespace(), snap_name, &exists);
...
int Image::snap_get_group_namespace(uint64_t snap_id,
  ...
  ImageCtx *ictx = (ImageCtx *)ctx;
  tracepoint(librbd, snap_get_group_namespace_enter, ictx, ictx->name.c_str());
  ...
  int r =
      librbd::api::Snapshot<>::get_group_namespace(ictx, snap_id, group_snap);
...
int Image::snap_get_mirror_namespace(uint64_t snap_id,
  ...
  int r =
      librbd::api::Snapshot<>::get_mirror_namespace(ictx, snap_id, mirror_snap);
  return r;
}
...
int Image::snap_set(const char *snap_name) {
  ImageCtx *ictx = (ImageCtx *)ctx;
  tracepoint(librbd, snap_set_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only, snap_name);
...
ssize_t Image::read(uint64_t ofs, size_t len, bufferlist &bl) {
  std::cout << __func__ << std::endl;
  ImageCtx *ictx = (ImageCtx *)ctx;
  tracepoint(librbd, read_enter, ictx, ictx->name.c_str(),
...
int64_t Image::read_iterate(uint64_t ofs, size_t len,
  ...
  std::cout << __func__ << std::endl;
  ImageCtx *ictx = (ImageCtx *)ctx;
  tracepoint(librbd, read_iterate_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only, ofs, len);
...
int Image::read_iterate2(uint64_t ofs, uint64_t len,
  ...
  std::cout << __func__ << std::endl;
  ImageCtx *ictx = (ImageCtx *)ctx;
  tracepoint(librbd, read_iterate2_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only, ofs, len);
...
int Image::diff_iterate(const char *fromsnapname, uint64_t ofs, uint64_t len,
  ...
  tracepoint(librbd, diff_iterate_enter, ictx, ictx->name.c_str(),
             ...
             true, false);
  int r = librbd::api::DiffIterate<>::diff_iterate(
      ictx, cls::rbd::UserSnapshotNamespace(), fromsnapname, ofs, len, true,
      false, cb, arg);
...
int Image::diff_iterate2(const char *fromsnapname, uint64_t ofs, uint64_t len,
  ...
  int r = librbd::api::DiffIterate<>::diff_iterate(
      ictx, cls::rbd::UserSnapshotNamespace(), fromsnapname, ofs, len,
...
ssize_t Image::write(uint64_t ofs, size_t len, bufferlist &bl) {
  ImageCtx *ictx = (ImageCtx *)ctx;
  tracepoint(librbd, write_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only, ofs, len,
             bl.length() < len ? NULL : bl.c_str());
  if (bl.length() < len) {
...
int Image::discard(uint64_t ofs, uint64_t len) {
  ...
  int r = api::Io<>::discard(*ictx, ofs, len, ictx->discard_granularity_bytes);
  tracepoint(librbd, discard_exit, r);
  return r;
}

ssize_t Image::writesame(uint64_t ofs, size_t len, bufferlist &bl,
                         int op_flags) {
  ImageCtx *ictx = (ImageCtx *)ctx;
  tracepoint(librbd, writesame_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only, ofs, len,
             bl.length() == 0 ? NULL : bl.c_str(), bl.length(), op_flags);
  ...
  bool discard_zero =
      ictx->config.get_val<bool>("rbd_discard_on_zeroed_write_same");
...
ssize_t Image::compare_and_write(uint64_t ofs, size_t len,
  ...
  tracepoint(librbd, compare_and_write_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only, ofs, len,
             cmp_bl.length() < len ? NULL : cmp_bl.c_str(),
             bl.length() < len ? NULL : bl.c_str(), op_flags);
  ...
  int r = api::Io<>::compare_and_write(*ictx, ofs, len, bufferlist{cmp_bl},
...
int Image::aio_read(uint64_t off, size_t len, bufferlist &bl,
  ...
  tracepoint(librbd, aio_read_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only, off, len, bl.c_str(),
             c->pc);
...
int Image::flush() {
  ImageCtx *ictx = (ImageCtx *)ctx;
  tracepoint(librbd, flush_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only);
...
int Image::aio_discard(uint64_t off, uint64_t len, RBD::AioCompletion *c) {
  ...
  api::Io<>::aio_discard(*ictx, get_aio_completion(c), off, len,
...
int Image::invalidate_cache() {
  ImageCtx *ictx = (ImageCtx *)ctx;
  tracepoint(librbd, invalidate_cache_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only);
...
int Image::metadata_list(const std::string &start, uint64_t max,
  ...
  if (r >= 0) {
    for (map<string, bufferlist>::iterator it = pairs->begin();
         it != pairs->end(); ++it) {
      tracepoint(librbd, metadata_list_entry, it->first.c_str(),
                 it->second.c_str());
    }
  }
...
int Image::mirror_image_get_status(mirror_image_status_t *mirror_image_status,
  ...
  *mirror_image_status = mirror_image_status_t{mirror_image_global_status.name,
                                               mirror_image_global_status.info,
                                               local_status.state,
                                               local_status.description,
                                               local_status.last_update,
                                               local_status.up};
...
int Image::list_watchers(std::list<librbd::image_watcher_t> &watchers) {
  ImageCtx *ictx = (ImageCtx *)ctx;
  tracepoint(librbd, list_watchers_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only);
  int r = librbd::list_watchers(ictx, watchers);
#ifdef WITH_LTTNG
  if (r >= 0) {
    for (auto &watcher : watchers) {
      tracepoint(librbd, list_watchers_entry, watcher.addr.c_str(), watcher.id,
                 watcher.cookie);
    }
  }
#endif
...
extern "C" int rbd_image_options_set_string(rbd_image_options_t opts,
                                            int optname, const char *optval) {
  return librbd::image_options_set(opts, optname, optval);
}

extern "C" int rbd_image_options_set_uint64(rbd_image_options_t opts,
                                            int optname, uint64_t optval) {
  return librbd::image_options_set(opts, optname, optval);
}

extern "C" int rbd_image_options_get_string(rbd_image_options_t opts,
...
extern "C" int rbd_image_options_get_uint64(rbd_image_options_t opts,
                                            int optname, uint64_t *optval) {
  return librbd::image_options_get(opts, optname, optval);
}
...
extern "C" int rbd_mirror_uuid_get(rados_ioctx_t p, char *mirror_uuid,
                                   size_t *max_len) {
  librados::IoCtx io_ctx;
  librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
...
extern "C" int rbd_mirror_peer_bootstrap_import(
    rados_ioctx_t p, rbd_mirror_peer_direction_t direction, const char *token) {
  librados::IoCtx io_ctx;
  librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
...
extern "C" int rbd_mirror_peer_site_list(rados_ioctx_t p,
...
extern "C" int rbd_mirror_peer_site_set_client_name(rados_ioctx_t p,
                                                    const char *uuid,
                                                    const char *client_name) {
  librados::IoCtx io_ctx;
  librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
...
extern "C" int rbd_mirror_peer_site_set_name(rados_ioctx_t p, const char *uuid,
                                             const char *site_name) {
  librados::IoCtx io_ctx;
  librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
  return librbd::api::Mirror<>::peer_site_set_name(io_ctx, uuid, site_name);
...
extern "C" int
rbd_mirror_peer_site_set_direction(rados_ioctx_t p, const char *uuid,
                                   rbd_mirror_peer_direction_t direction) {
  librados::IoCtx io_ctx;
  librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
  return librbd::api::Mirror<>::peer_site_set_direction(io_ctx, uuid,
...
extern "C" int rbd_mirror_peer_site_get_attributes(
  ...
  int r = librbd::api::Mirror<>::peer_site_get_attributes(io_ctx, uuid,
                                                          &attributes);
  if (r < 0) {
    return r;
  }
  ...
  bool too_short =
      ((*max_key_len < key_total_len) || (*max_val_len < val_total_len));
...
extern "C" int rbd_mirror_image_global_status_list(
...
extern "C" int rbd_mirror_peer_add(rados_ioctx_t p, char *uuid,
                                   ...
                                   const char *client_name) {
  return rbd_mirror_peer_site_add(p, uuid, uuid_max_length,
                                  RBD_MIRROR_PEER_DIRECTION_RX_TX, cluster_name,
                                  client_name);
}
...
extern "C" int rbd_mirror_peer_list(rados_ioctx_t p, rbd_mirror_peer_t *peers,
                                    int *max_peers) {
  librados::IoCtx io_ctx;
  librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
...
extern "C" int rbd_mirror_peer_get_attributes(rados_ioctx_t p, const char *uuid,
                                              char *keys, size_t *max_key_len,
                                              char *values, size_t *max_val_len,
                                              size_t *key_value_count) {
  return rbd_mirror_peer_site_get_attributes(p, uuid, keys, max_key_len, values,
                                             max_val_len, key_value_count);
}

extern "C" int rbd_mirror_peer_set_attributes(rados_ioctx_t p, const char *uuid,
                                              const char *keys,
                                              const char *values,
                                              size_t count) {
  return rbd_mirror_peer_site_set_attributes(p, uuid, keys, values, count);
}
...
extern "C" void rbd_mirror_image_status_list_cleanup(
...
extern "C" int
rbd_mirror_image_status_summary(rados_ioctx_t p,
...
extern "C" int rbd_mirror_image_instance_id_list(rados_ioctx_t p,
                                                 const char *start_id,
...
extern "C" void rbd_mirror_image_instance_id_list_cleanup(char **image_ids,
                                                          char **instance_ids,
                                                          size_t len) {
  for (size_t i = 0; i < len; i++) {
    free(image_ids[i]);
...
extern "C" int rbd_mirror_image_info_list(
    rados_ioctx_t p, rbd_mirror_image_mode_t *mode_filter, const char *start_id,
    size_t max, char **image_ids, rbd_mirror_image_mode_t *mode_entries,
    rbd_mirror_image_info_t *info_entries, size_t *num_entries) {
  librados::IoCtx io_ctx;
  librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
  std::map<std::string,
           std::pair<librbd::mirror_image_mode_t, librbd::mirror_image_info_t>>
      cpp_entries;
...
extern "C" void
rbd_mirror_image_info_list_cleanup(char **image_ids,
...
extern "C" void
rbd_linked_image_spec_list_cleanup(rbd_linked_image_spec_t *images,
                                   size_t num_images) {
  for (size_t idx = 0; idx < num_images; ++idx) {
...
extern "C" int rbd_create2(rados_ioctx_t p, const char *name, uint64_t size,
                           uint64_t features, int *order) {
  librados::IoCtx io_ctx;
  librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
...
extern "C" int rbd_create3(rados_ioctx_t p, const char *name, uint64_t size,
                           uint64_t features, int *order, uint64_t stripe_unit,
                           uint64_t stripe_count) {
  librados::IoCtx io_ctx;
  librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
...
extern "C" int rbd_create4(rados_ioctx_t p, const char *name, uint64_t size,
                           rbd_image_options_t opts) {
  librados::IoCtx io_ctx;
  librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
  tracepoint(librbd, create4_enter, io_ctx.get_pool_name().c_str(),
             io_ctx.get_id(), name, size, opts);
...
extern "C" int rbd_clone(rados_ioctx_t p_ioctx, const char *p_name,
  ...
  tracepoint(librbd, clone_enter, p_ioc.get_pool_name().c_str(), p_ioc.get_id(),
             p_name, p_snap_name, c_ioc.get_pool_name().c_str(), c_ioc.get_id(),
             c_name, features);
  int r = librbd::clone(p_ioc, p_name, p_snap_name, c_ioc, c_name, features,
                        c_order, 0, 0);
  tracepoint(librbd, clone_exit, r, *c_order);
  return r;
}
...
extern "C" int rbd_clone2(rados_ioctx_t p_ioctx, const char *p_name,
  ...
  librados::IoCtx::from_rados_ioctx_t(c_ioctx, c_ioc);
  TracepointProvider::initialize<tracepoint_traits>(get_cct(p_ioc));
  tracepoint(librbd, clone2_enter, p_ioc.get_pool_name().c_str(),
             p_ioc.get_id(), p_name, p_snap_name, c_ioc.get_pool_name().c_str(),
             c_ioc.get_id(), c_name, features, stripe_unit, stripe_count);
  int r = librbd::clone(p_ioc, p_name, p_snap_name, c_ioc, c_name, features,
                        c_order, stripe_unit, stripe_count);
  tracepoint(librbd, clone2_exit, r, *c_order);
  return r;
...
extern "C" int rbd_clone3(rados_ioctx_t p_ioctx, const char *p_name,
  ...
  librados::IoCtx::from_rados_ioctx_t(c_ioctx, c_ioc);
  TracepointProvider::initialize<tracepoint_traits>(get_cct(p_ioc));
  tracepoint(librbd, clone3_enter, p_ioc.get_pool_name().c_str(),
             p_ioc.get_id(), p_name, p_snap_name, c_ioc.get_pool_name().c_str(),
...
extern "C" int rbd_trash_remove_with_progress(rados_ioctx_t p,
                                              const char *image_id, bool force,
                                              librbd_progress_fn_t cb,
                                              void *cbdata) {
  librados::IoCtx io_ctx;
  librados::IoCtx::from_rados_ioctx_t(p, io_ctx);
...
extern "C" int rbd_namespace_exists(rados_ioctx_t io,
                                    const char *namespace_name, bool *exists) {
  librados::IoCtx io_ctx;
  librados::IoCtx::from_rados_ioctx_t(io, io_ctx);
...
extern "C" int rbd_pool_stats_get(rados_ioctx_t io,
...
extern "C" int rbd_copy2(rbd_image_t srcp, rbd_image_t destp) {
  ...
  tracepoint(librbd, copy2_enter, src, src->name.c_str(),
             src->snap_name.c_str(), src->read_only, dest, dest->name.c_str(),
             dest->snap_name.c_str(), dest->read_only);
  librbd::NoOpProgressContext prog_ctx;
...
extern "C" int rbd_copy4(rbd_image_t image, rados_ioctx_t dest_p,
  ...
  tracepoint(librbd, copy4_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only,
             dest_io_ctx.get_pool_name().c_str(), dest_io_ctx.get_id(),
             destname, c_opts, sparse_size);
  librbd::ImageOptions c_opts_(c_opts);
  librbd::NoOpProgressContext prog_ctx;
  int r =
      librbd::copy(ictx, dest_io_ctx, destname, c_opts_, prog_ctx, sparse_size);
  tracepoint(librbd, copy4_exit, r);
  return r;
}
...
extern "C" int rbd_copy_with_progress4(rbd_image_t image, rados_ioctx_t dest_p,
  ...
  tracepoint(librbd, copy4_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only,
             dest_io_ctx.get_pool_name().c_str(), dest_io_ctx.get_id(),
             destname, dest_opts, sparse_size);
  librbd::ImageOptions dest_opts_(dest_opts);
  librbd::CProgressContext prog_ctx(fn, data);
...
extern "C" int
rbd_deep_copy_with_progress(rbd_image_t image, rados_ioctx_t dest_p,
...
extern "C" int rbd_migration_prepare_import(const char *source_spec,
                                            rados_ioctx_t dest_p,
                                            const char *dest_image_name,
                                            rbd_image_options_t opts_) {
  librados::IoCtx dest_io_ctx;
...
extern "C" int rbd_open(rados_ioctx_t p, const char *name, rbd_image_t *image,
  ...
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
  librbd::ImageCtx *ictx =
      new librbd::ImageCtx(name, "", snap_name, io_ctx, false);
  tracepoint(librbd, open_image_enter, ictx, ictx->name.c_str(),
             ictx->id.c_str(), ictx->snap_name.c_str(), ictx->read_only);
...
extern "C" int rbd_open_by_id(rados_ioctx_t p, const char *id,
  ...
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
  librbd::ImageCtx *ictx =
      new librbd::ImageCtx("", id, snap_name, io_ctx, false);
  tracepoint(librbd, open_image_enter, ictx, ictx->name.c_str(),
             ictx->id.c_str(), ictx->snap_name.c_str(), ictx->read_only);
...
extern "C" int rbd_aio_open(rados_ioctx_t p, const char *name,
  ...
  tracepoint(librbd, aio_open_image_enter, ictx, ictx->name.c_str(),
             ...
             comp->pc);
  ictx->state->open(0,
                    new C_OpenComplete(ictx, get_aio_completion(comp), image));
  tracepoint(librbd, aio_open_image_exit, 0);
  return 0;
}
...
extern "C" int rbd_open_read_only(rados_ioctx_t p, const char *name,
  ...
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
  librbd::ImageCtx *ictx =
      new librbd::ImageCtx(name, "", snap_name, io_ctx, true);
  tracepoint(librbd, open_image_enter, ictx, ictx->name.c_str(),
             ictx->id.c_str(), ictx->snap_name.c_str(), ictx->read_only);
...
extern "C" int rbd_open_by_id_read_only(rados_ioctx_t p, const char *id,
  ...
  TracepointProvider::initialize<tracepoint_traits>(get_cct(io_ctx));
  librbd::ImageCtx *ictx =
      new librbd::ImageCtx("", id, snap_name, io_ctx, true);
  tracepoint(librbd, open_image_enter, ictx, ictx->name.c_str(),
             ictx->id.c_str(), ictx->snap_name.c_str(), ictx->read_only);
...
extern "C" int rbd_features_to_string(uint64_t features, char *str_features,
                                      size_t *size) {
  std::stringstream err;
...
extern "C" int rbd_features_from_string(const char *str_features,
                                        uint64_t *features) {
  std::stringstream err;
  *features = librbd::rbd_features_from_string(str_features, &err);
  if (!err.str().empty()) {
...
extern "C" int rbd_get_parent_info(rbd_image_t image, char *parent_pool_name,
...
extern "C" int rbd_get_parent_info2(rbd_image_t image, char *parent_pool_name,
                                    size_t ppool_namelen, char *parent_name,
                                    size_t pnamelen, char *parent_id,
                                    size_t pidlen, char *parent_snap_name,
                                    size_t psnap_namelen) {
  auto ictx = reinterpret_cast<librbd::ImageCtx *>(image);
  tracepoint(librbd, get_parent_info_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only);
...
extern "C" int rbd_get_parent(rbd_image_t image,
  ...
  if (r < 0) {
    ...
    *parent_image = {.pool_id = cpp_parent_image.pool_id,
                     .pool_name = strdup(cpp_parent_image.pool_name.c_str()),
                     .pool_namespace =
                         strdup(cpp_parent_image.pool_namespace.c_str()),
    ...
    *parent_snap = {.id = cpp_parent_snap.id,
                    .namespace_type = cpp_parent_snap.namespace_type,
                    .name = strdup(cpp_parent_snap.name.c_str())};
  }

  tracepoint(librbd, get_parent_info_exit, r, parent_image->pool_name,
             parent_image->image_name, parent_image->image_id,
             parent_snap->name);
  return r;
}
...
extern "C" int rbd_snap_rename(rbd_image_t image, const char *srcname,
                               const char *dstname) {
  librbd::ImageCtx *ictx = (librbd::ImageCtx *)image;
  tracepoint(librbd, snap_rename_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only, srcname, dstname);
...
extern "C" int rbd_snap_remove2(rbd_image_t image, const char *snap_name,
                                uint32_t flags, librbd_progress_fn_t cb,
                                void *cbdata) {
  librbd::ImageCtx *ictx = (librbd::ImageCtx *)image;
  tracepoint(librbd, snap_remove2_enter, ictx, ictx->name.c_str(),
...
extern "C" int rbd_snap_is_protected(rbd_image_t image, const char *snap_name,
  ...
  bool protected_snap;
  int r =
      librbd::api::Snapshot<>::is_protected(ictx, snap_name, &protected_snap);
  if (r < 0) {
    tracepoint(librbd, snap_is_protected_exit, r, *is_protected ? 1 : 0);
    return r;
  }
...
extern "C" int rbd_snap_exists(rbd_image_t image, const char *snapname,
                               bool *exists) {
  librbd::ImageCtx *ictx = (librbd::ImageCtx *)image;
  tracepoint(librbd, snap_exists_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only, snapname);
  int r = librbd::api::Snapshot<>::exists(
      ictx, cls::rbd::UserSnapshotNamespace(), snapname, exists);
  tracepoint(librbd, snap_exists_exit, r, *exists);
  return r;
}

extern "C" int rbd_snap_get_timestamp(rbd_image_t image, uint64_t snap_id,
                                      struct timespec *timestamp) {
  librbd::ImageCtx *ictx = (librbd::ImageCtx *)image;
  tracepoint(librbd, snap_get_timestamp_enter, ictx, ictx->name.c_str());
  int r = librbd::api::Snapshot<>::get_timestamp(ictx, snap_id, timestamp);
...
extern "C" int rbd_snap_get_name(rbd_image_t image, uint64_t snap_id,
...
extern "C" int rbd_snap_get_id(rbd_image_t image, const char *snapname,
                               uint64_t *snap_id) {
  librbd::ImageCtx *ictx = (librbd::ImageCtx *)image;
...
extern "C" int rbd_list_children2(rbd_image_t image, rbd_child_info_t *children,
                                  int *max_children) {
  auto ictx = reinterpret_cast<librbd::ImageCtx *>(image);
  tracepoint(librbd, list_children_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only);
...
extern "C" int rbd_list_children3(rbd_image_t image,
  ...
  *max_images = cpp_children.size();
  for (size_t idx = 0; idx < cpp_children.size(); ++idx) {
    images[idx] = {.pool_id = cpp_children[idx].pool_id,
                   .pool_name = strdup(cpp_children[idx].pool_name.c_str()),
                   .pool_namespace =
                       strdup(cpp_children[idx].pool_namespace.c_str()),
...
extern "C" ssize_t rbd_list_lockers(rbd_image_t image, int *exclusive,
                                    char *tag, size_t *tag_len, char *clients,
                                    size_t *clients_len, char *cookies,
                                    size_t *cookies_len, char *addrs,
                                    size_t *addrs_len) {
  librbd::ImageCtx *ictx = (librbd::ImageCtx *)image;
  tracepoint(librbd, list_lockers_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only);
  ...
  ldout(ictx->cct, 20) << "list_lockers r = " << r
                       << " lockers.size() = " << lockers.size() << dendl;
  ...
  *exclusive = (int)exclusive_bool;
  ...
  bool too_short =
      ((clients_total > *clients_len) || (cookies_total > *cookies_len) ||
       (addrs_total > *addrs_len) || (tag_str.length() + 1 > *tag_len));
...
extern "C" int rbd_lock_exclusive(rbd_image_t image, const char *cookie) {
  librbd::ImageCtx *ictx = (librbd::ImageCtx *)image;
  tracepoint(librbd, lock_exclusive_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only, cookie);
...
extern "C" int rbd_lock_shared(rbd_image_t image, const char *cookie,
  ...
  librbd::ImageCtx *ictx = (librbd::ImageCtx *)image;
  tracepoint(librbd, lock_shared_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only, cookie, tag);
...
extern "C" int rbd_break_lock(rbd_image_t image, const char *client,
  ...
  librbd::ImageCtx *ictx = (librbd::ImageCtx *)image;
  tracepoint(librbd, break_lock_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only, client, cookie);
...
extern "C" ssize_t rbd_read(rbd_image_t image, uint64_t ofs, size_t len,
  ...
  tracepoint(librbd, read_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only, ofs, len);
  int r = librbd::api::Io<>::read(*ictx, ofs, len,
...
extern "C" int64_t
rbd_read_iterate(rbd_image_t image, uint64_t ofs, size_t len,
...
extern "C" int rbd_read_iterate2(rbd_image_t image, uint64_t ofs, uint64_t len,
                                 int (*cb)(uint64_t, size_t, const char *,
                                           void *),
                                 void *arg) {
  std::cout << __func__ << std::endl;
  librbd::ImageCtx *ictx = (librbd::ImageCtx *)image;
...
extern "C" int rbd_diff_iterate(rbd_image_t image, const char *fromsnapname,
...
extern "C" ssize_t rbd_write(rbd_image_t image, uint64_t ofs, size_t len,
  ...
  librbd::ImageCtx *ictx = (librbd::ImageCtx *)image;
  tracepoint(librbd, write_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only, ofs, len, buf);
...
extern "C" int rbd_discard(rbd_image_t image, uint64_t ofs, uint64_t len) {
  ...
  int r = librbd::api::Io<>::discard(*ictx, ofs, len,
...
extern "C" ssize_t rbd_writesame(rbd_image_t image, uint64_t ofs, size_t len,
  ...
  tracepoint(librbd, writesame_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only, ofs, len,
             data_len == 0 ? NULL : buf, data_len, op_flags);
  ...
  int r =
      librbd::api::Io<>::write_same(*ictx, ofs, len, std::move(bl), op_flags);
  tracepoint(librbd, writesame_exit, r);
  return r;
}
...
extern "C" ssize_t rbd_compare_and_write(rbd_image_t image, uint64_t ofs,
                                         size_t len, const char *cmp_buf,
                                         const char *buf,
                                         uint64_t *mismatch_off, int op_flags) {
  librbd::ImageCtx *ictx = (librbd::ImageCtx *)image;
  tracepoint(librbd, compare_and_write_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only, ofs, len, cmp_buf, buf,
             op_flags);
  ...
  int r = librbd::api::Io<>::compare_and_write(*ictx, ofs, len,
                                               std::move(cmp_bl), std::move(bl),
                                               mismatch_off, op_flags);
  tracepoint(librbd, compare_and_write_exit, r);
  return r;
}
...
extern "C" int rbd_aio_write(rbd_image_t image, uint64_t off, size_t len,
  ...
  librbd::RBD::AioCompletion *comp = (librbd::RBD::AioCompletion *)c;
  tracepoint(librbd, aio_write_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only, off, len, buf, comp->pc);

  auto aio_completion = get_aio_completion(comp);
  bufferlist bl;
  bl.push_back(create_write_raw(ictx, buf, len, aio_completion));
  librbd::api::Io<>::aio_write(*ictx, aio_completion, off, len, std::move(bl),
                               0, true);
  tracepoint(librbd, aio_write_exit, 0);
  return 0;
}
...
extern "C" int rbd_aio_read(rbd_image_t image, uint64_t off, size_t len,
  ...
  tracepoint(librbd, aio_read_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only, off, len, buf, comp->pc);
  librbd::api::Io<>::aio_read(*ictx, get_aio_completion(comp), off, len,
                              librbd::io::ReadResult{buf, len}, 0, true);
  tracepoint(librbd, aio_read_exit, 0);
  return 0;
}
...
extern "C" int rbd_aio_read2(rbd_image_t image, uint64_t off, size_t len,
  ...
  tracepoint(librbd, aio_read2_enter, ictx, ictx->name.c_str(),
             ...
             op_flags);
  librbd::api::Io<>::aio_read(*ictx, get_aio_completion(comp), off, len,
                              librbd::io::ReadResult{buf, len}, op_flags, true);
  tracepoint(librbd, aio_read_exit, 0);
  return 0;
}
...
extern "C" int rbd_aio_readv(rbd_image_t image, const struct iovec *iov,
  ...
  if (r == 0) {
    ...
    if (iovcnt == 1) {
      read_result = librbd::io::ReadResult(static_cast<char *>(iov[0].iov_base),
                                           iov[0].iov_len);
    } else {
...
extern "C" int rbd_aio_discard(rbd_image_t image, uint64_t off, uint64_t len,
  ...
  librbd::RBD::AioCompletion *comp = (librbd::RBD::AioCompletion *)c;
  tracepoint(librbd, aio_discard_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only, off, len, comp->pc);
  librbd::api::Io<>::aio_discard(*ictx, get_aio_completion(comp), off, len,
                                 ictx->discard_granularity_bytes, true);
  tracepoint(librbd, aio_discard_exit, 0);
  return 0;
}
...
extern "C" int rbd_aio_writesame(rbd_image_t image, uint64_t off, size_t len,
  ...
  tracepoint(librbd, aio_writesame_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only, off, len,
             data_len == 0 ? NULL : buf, data_len, comp->pc, op_flags);
  ...
  bool discard_zero =
      ictx->config.get_val<bool>("rbd_discard_on_zeroed_write_same");
  if (discard_zero && mem_is_zero(buf, data_len)) {
    librbd::api::Io<>::aio_write_zeroes(*ictx, get_aio_completion(comp), off,
                                        len, 0, op_flags, true);
    tracepoint(librbd, aio_writesame_exit, 0);
    return 0;
  }
  ...
  librbd::api::Io<>::aio_write_same(*ictx, aio_completion, off, len,
                                    std::move(bl), op_flags, true);
  tracepoint(librbd, aio_writesame_exit, 0);
  return 0;
}
...
extern "C" ssize_t rbd_aio_compare_and_write(
    rbd_image_t image, uint64_t off, size_t len, const char *cmp_buf,
    const char *buf, rbd_completion_t c, uint64_t *mismatch_off, int op_flags) {
  librbd::ImageCtx *ictx = (librbd::ImageCtx *)image;
  librbd::RBD::AioCompletion *comp = (librbd::RBD::AioCompletion *)c;
  tracepoint(librbd, aio_compare_and_write_enter, ictx, ictx->name.c_str(),
             ictx->snap_name.c_str(), ictx->read_only, off, len, cmp_buf, buf,
             comp->pc, op_flags);
  ...
  bufferlist bl;
  bl.push_back(create_write_raw(ictx, buf, len, aio_completion));
  librbd::api::Io<>::aio_compare_and_write(*ictx, aio_completion, off, len,
                                           std::move(cmp_bl), std::move(bl),
                                           mismatch_off, op_flags, false);
...
extern "C" int rbd_metadata_get(rbd_image_t image, const char *key, char *value,
                                size_t *vallen) {
  librbd::ImageCtx *ictx = (librbd::ImageCtx *)image;
  string val_s;
...
extern "C" int rbd_metadata_set(rbd_image_t image, const char *key,
                                const char *value) {
  librbd::ImageCtx *ictx = (librbd::ImageCtx *)image;
...
extern "C" int rbd_metadata_list(rbd_image_t image, const char *start,
                                 uint64_t max, char *key, size_t *key_len,
                                 char *value, size_t *val_len) {
  librbd::ImageCtx *ictx = (librbd::ImageCtx *)image;
  tracepoint(librbd, metadata_list_enter, ictx);
  ...
  size_t key_total_len = 0, val_total_len = 0;
  bool too_short = false;
  for (map<string, bufferlist>::iterator it = pairs.begin(); it != pairs.end();
       ++it) {
    key_total_len += it->first.size() + 1;
    val_total_len += it->second.length() + 1;
  }
...
extern "C" int
rbd_mirror_image_get_global_status(rbd_image_t image,
                                   rbd_mirror_image_global_status_t *status,
                                   size_t status_size) {
  librbd::ImageCtx *ictx = (librbd::ImageCtx *)image;
...
extern "C" int
rbd_mirror_image_get_instance_id(rbd_image_t image, char *instance_id,
...
extern "C" int rbd_aio_mirror_image_get_info(rbd_image_t image,
  ...
  auto ctx = new C_MirrorImageGetInfo(
      info, new C_AioCompletion(ictx, librbd::io::AIO_TYPE_GENERIC,
                                get_aio_completion(comp)));
  librbd::api::Mirror<>::image_get_info(ictx, &ctx->cpp_mirror_image_info, ctx);
  return 0;
}
...
extern "C" int
rbd_aio_mirror_image_get_global_status(rbd_image_t image,
                                       rbd_mirror_image_global_status_t *status,
...
extern "C" int rbd_group_image_list(rados_ioctx_t group_p,
  ...
  int r =
      librbd::api::Group<>::image_list(group_ioctx, group_name, &cpp_images);
...
extern "C" int
rbd_snap_get_namespace_type(rbd_image_t image, uint64_t snap_id,
...
extern "C" int
rbd_snap_get_group_namespace(rbd_image_t image, uint64_t snap_id,
...
extern "C" int
rbd_snap_get_mirror_namespace(rbd_image_t image, uint64_t snap_id,
  ...
  int r = librbd::api::Snapshot<>::get_mirror_namespace(ictx, snap_id,
                                                        &mirror_namespace);
  if (r < 0) {
    return r;
  }
...
extern "C" int
rbd_snap_mirror_namespace_cleanup(rbd_snap_mirror_namespace_t *mirror_snap,
                                  size_t mirror_snap_size) {
  if (mirror_snap_size != sizeof(rbd_snap_mirror_namespace_t)) {
    return -ERANGE;
  }
...
extern "C" void rbd_quiesce_complete(rbd_image_t image, uint64_t handle,
                                     int r) {
  librbd::ImageCtx *ictx = (librbd::ImageCtx *)image;
  ictx->state->quiesce_complete(handle, r);
}
