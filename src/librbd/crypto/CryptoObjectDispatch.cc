...
#undef dout_prefix
#define dout_prefix                                                            \
  *_dout << "librbd::crypto::CryptoObjectDispatch: " << this << " "            \
         << __func__ << ": "
...
template <typename I> struct C_AlignedObjectReadRequest : public Context {
  I *image_ctx;
  ...
  C_AlignedObjectReadRequest(I *image_ctx, ceph::ref_t<CryptoInterface> crypto,
      ...
      : image_ctx(image_ctx), crypto(crypto), object_no(object_no),
        extents(extents), io_context(io_context), parent_trace(parent_trace),
    ...
    auto ctx =
        create_context_callback<C_AlignedObjectReadRequest<I>,
                                &C_AlignedObjectReadRequest<I>::handle_read>(
            this);

    req = io::ObjectDispatchSpec::create_read(
        image_ctx, io::OBJECT_DISPATCH_LAYER_CRYPTO, object_no, extents,
        io_context, op_flags, read_flags, parent_trace, version, ctx);
  }

  void send() { req->send(); }

  void finish(int r) override { on_finish->complete(r); }
  ...
  void handle_read(int r) {
    ...
    if (r == 0) {
      for (auto &extent : *extents) {
        auto crypto_ret = crypto->decrypt_aligned_extent(
            extent, Striper::get_file_offset(cct, &image_ctx->layout, object_no,
                                             extent.offset));
        if (crypto_ret != 0) {
    ...
    if (r == -ENOENT && !disable_read_from_parent) {
      io::util::read_parent<I>(image_ctx, object_no, extents,
...
template <typename I> struct C_UnalignedObjectReadRequest : public Context {
  CephContext *cct;
  io::ReadExtents *extents;
  Context *on_finish;
  ...
  C_UnalignedObjectReadRequest(
      I *image_ctx, ceph::ref_t<CryptoInterface> crypto, uint64_t object_no,
      io::ReadExtents *extents, IOContext io_context, int op_flags,
      int read_flags, const ZTracer::Trace &parent_trace, uint64_t *version,
      int *object_dispatch_flags, Context *on_dispatched)
      : cct(image_ctx->cct), extents(extents), on_finish(on_dispatched) {
    crypto->align_extents(*extents, &aligned_extents);

    // send the aligned read back to get decrypted
    req = io::ObjectDispatchSpec::create_read(
        image_ctx, io::OBJECT_DISPATCH_LAYER_NONE, object_no, &aligned_extents,
...
template <typename I> struct C_UnalignedObjectWriteRequest : public Context {
  I *image_ctx;
  ceph::ref_t<CryptoInterface> crypto;
  uint64_t object_no;
  uint64_t object_off;
  ...
  C_UnalignedObjectWriteRequest(
      I *image_ctx, ceph::ref_t<CryptoInterface> crypto, uint64_t object_no,
      uint64_t object_off, ceph::bufferlist &&data, ceph::bufferlist &&cmp_data,
      uint64_t *mismatch_offset, IOContext io_context, int op_flags,
      ...
      : image_ctx(image_ctx), crypto(crypto), object_no(object_no),
        ...
        object_dispatch_flags(object_dispatch_flags), journal_tid(journal_tid),
    ...
    auto ctx =
        create_context_callback<C_UnalignedObjectWriteRequest<I>,
                                &C_UnalignedObjectWriteRequest<I>::handle_read>(
            this);

    read_req = new C_UnalignedObjectReadRequest<I>(
        image_ctx, crypto, object_no, &extents, io_context, 0,
        io::READ_FLAG_DISABLE_READ_FROM_PARENT, parent_trace, &version, 0, ctx);
  }

  void send() { read_req->send(); }
  ...
  bool check_cmp_data() {
    ...
    io::util::unsparsify(image_ctx->cct, &cmp_extent.bl, cmp_extent.extent_map,
                         cmp_extent.offset, cmp_extent.length);
  ...
  void handle_read(int r) {
    ...
    auto write_req = io::ObjectDispatchSpec::create_write(
        image_ctx, io::OBJECT_DISPATCH_LAYER_NONE, object_no, aligned_off,
  ...
  void restart_request() {
    auto req = new C_UnalignedObjectWriteRequest<I>(
        image_ctx, crypto, object_no, object_off, std::move(data),
        std::move(cmp_data), mismatch_offset, io_context, op_flags, write_flags,
        assert_version, parent_trace, object_dispatch_flags, journal_tid, this);
    req->send();
  }
...
template <typename I> void CryptoObjectDispatch<I>::init(Context *on_finish) {
  ...
  *dispatch_result = io::DISPATCH_RESULT_COMPLETE;
  if (m_crypto->is_aligned(*extents)) {
    auto req = new C_AlignedObjectReadRequest<I>(
        ...
        read_flags, parent_trace, version, object_dispatch_flags,
        on_dispatched);
    req->send();
  } else {
...
bool CryptoObjectDispatch<I>::write(
  ...
  if (m_crypto->is_aligned(object_off, data.length())) {
    auto r = m_crypto->encrypt(
        &data, Striper::get_file_offset(m_image_ctx->cct, &m_image_ctx->layout,
                                        object_no, object_off));
    *dispatch_result =
        r == 0 ? io::DISPATCH_RESULT_CONTINUE : io::DISPATCH_RESULT_COMPLETE;
...
bool CryptoObjectDispatch<I>::write_same(
  ...
  auto ctx = new LambdaContext(
      [on_finish_ctx = on_dispatched](int r) { on_finish_ctx->complete(r); });
